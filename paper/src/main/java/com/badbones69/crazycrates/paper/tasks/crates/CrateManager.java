...
public class CrateManager {
    ...
    private ItemBuilder getKey(@NotNull final FileConfiguration file) {
        ...
        final ItemBuilder itemBuilder = file.contains("Crate.PhysicalKey.Data") ? ItemBuilder.from(file.getString("Crate.PhysicalKey.Data", "")) : ItemBuilder.from(file.getString("Crate.PhysicalKey.Item", "tripwire_hook").toLowerCase());

        if (hideFlags) {
            itemBuilder.hideToolTip();
        }

        return itemBuilder.setDisplayName(name).withDisplayLore(lore).setEnchantGlint(glowing).setItemModel(namespace, id).setCustomModelData(customModelData);
    }
