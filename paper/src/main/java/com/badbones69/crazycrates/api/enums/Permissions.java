...
public enum Permissions {
    ...
    CRAZYCRATES_OPEN_OTHER("open-others", "Tries to open a crate for a player if they have a key.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
    ...
    CRAZYCRATES_FORCE_OPEN("forceopen", "Opens a crate for a player for free.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
    ...
    CRAZYCRATES_GIVE_ALL("giveall", "Gives all online players keys to use on crates.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
        put("crazycrates.giverandomkey", true);
        put("crazycrates.transfer", true);
        put("crazycrates.givekey", true);
        put("crazycrates.takekey", true);
    }}),
    CRAZYCRATES_TAKE_KEY("takekey", "Allows you to take keys from a player.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
        put("crazycrates.transfer", true);
    }}),
    CRAZYCRATES_SET_CRATE("set", "Set the block you are looking at as the specified crate.", PermissionDefault.OP),
    CRAZYCRATES_SET_MENU("setmenu", "Sets the block you are looking at to open the (/crate) crate menu.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
    ...
    CRAZYCRATES_LIST("list", "Displays a list of all available crates.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
        put("crazycrates.teleport", true);
    }}),
    CRAZYCRATES_TELEPORT("teleport", "Teleports to a crate.", PermissionDefault.OP),
    CRAZYCRATES_KEY_ALL("keys-others", "Check the number of keys a player has.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
        put("crazycrates.keys", true);
    }}),
    CRAZYCRATES_ADMIN_ALL("admin.*", "Give all admin based permissions.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
        put("crazycrates.open-others", true);
    ...
    CRAZYCRATES_PLAYER_ALL("players.*", "Give all player based permissions.", PermissionDefault.OP, new ConcurrentHashMap<>() {{
     ...
     */
    Permissions(@NotNull final String node, @NotNull final String description, @NotNull final PermissionDefault isDefault, @NotNull final ConcurrentHashMap<String, Boolean> children) {
        this.node = node;
