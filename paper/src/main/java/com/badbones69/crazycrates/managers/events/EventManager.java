...
public class EventManager {
    ...
    public static void logEvent(final EventType type, final Player player, final CommandSender sender, final Crate crate, final KeyType keyType) {
        handle(type, player, sender, crate, keyType);
    }

    public static void logEvent(final EventType type, final OfflinePlayer player, final CommandSender sender, final Crate crate, final KeyType keyType) {
        handle(type, player, sender, crate, keyType);
    }

    private static void handle(EventType type, OfflinePlayer player, CommandSender sender, Crate crate, KeyType keyType) {
        String message = "";
        File file = null;

        switch (type) {
            case event_key_given, event_key_removed, event_key_received, event_key_sent, event_key_taken -> {
            ...
            case event_crate_opened -> {
                message = "Player: %player% | Crate Name: %crate_name% | Crate Type: %crate_type% | Key Name: %key_name% | Key Type: %key_type% | Key Item: %key_item%"
                        .replace("%player%", player.getName())
    ...
    private static void log(final String message, final File file, final EventType type) {
        ...
        if (log_to_console) {
            plugin.getLogger().info("[" + time + " " + type.getEvent() + "]: " + message);
        }
    }
}