...
public class Methods {
    ...
    private final us.crazycrew.crazycrates.paper.CrazyCrates plugin;

    public Methods(us.crazycrew.crazycrates.paper.CrazyCrates plugin) {
        this.plugin = plugin;
    }

    private final @NotNull CrazyHandler crazyHandler = null;
    private final @NotNull ConfigManager configManager = null;
    ...
    public void sendMessage(CommandSender commandSender, String message, boolean prefixToggle) {
        ...
        if (commandSender instanceof Player player) {
            if (!prefix.isEmpty() && prefixToggle) player.sendMessage(LegacyUtils.color(message.replaceAll("%prefix%",
                    quoteReplacement(prefix))).replaceAll("%Prefix%", quoteReplacement(prefix))); else player.sendMessage(
                            LegacyUtils.color(message));

            return;
        }

        if (!prefix.isEmpty() && prefixToggle) commandSender.sendMessage(LegacyUtils.color(message.replaceAll("%prefix%",
                quoteReplacement(prefix))).replaceAll("%Prefix%", quoteReplacement(prefix))); else commandSender.sendMessage(
                        LegacyUtils.color(message));
    }
    ...
    public void addFirework(Entity firework) {
        ...
        //container.set(PersistentKeys.no_firework_damage, PersistentDataType.BOOLEAN, true);
    }
    ...
    public void checkPrize(Prize prize, CrazyManager crazyManager, CrazyCratesOld plugin, Player player, Crate crate) {
        if (prize != null) {
