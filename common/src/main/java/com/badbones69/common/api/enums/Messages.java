...
public enum Messages {
    ...
    command_opened_crate("command.open.opened-a-crate", "{prefix}<gray>You have opened the {crate} <gray>for <gold>{player}.", "command", "open", "opened-a-crate"),
    command_gave_player_keys("command.give.given-player-keys", "{prefix}<gray>You have given <gold>{player} {amount} <gray>key(s).", "command", "give", "given-player-keys"),
    command_cannot_give_player_keys("command.give.full-inventory", "{prefix}<gray>You have been given <gold>{amount} {key} <gray>virtual key(s) because your inventory was full.", "command", "give", "full-inventory"),
    command_gave_everyone_keys("command.give.given-everyone-keys", "{prefix}<gray>You have given everyone <gold>{amount} <gray>key(s).", "command", "give", "given-everyone-keys"),
    command_gave_offline_player_keys("command.give.given-offline-player-keys", "{prefix}<gray>You have given <gold>{amount} <gray>key(s) to the offline player <gold>{player}.", "command", "give", "given-offline-player-keys"),
    command_take_player_keys("command.take.take-player-keys", "{prefix}<gray>You have taken <gold>{amount} <gray>key(s) from <gold>{player}.", "command", "take", "take-player-keys"),
    command_cant_take_keys("command.take.cannot-take-keys", "{prefix}<gray>You cannot take key(s) from <gold>{player} <gray>as they are poor.", "command", "take", "cannot-take-keys"),
    command_take_offline_player_keys("command.take.take-offline-player-keys", "{prefix}<gray>You have taken <gold>{amount} <gray>key(s) from the offline player <gold>{player}.", "command", "take", "take-offline-player-keys"),

    command_editor_item_not_in_hand("command.additem.no-item-in-hand", "{prefix}<red>You need to have an item in your hand to add it {crate}.", "command", "additem", "no-item-in-hand"),
    command_editor_item_added("command.additem.add-item-from-hand", "{prefix}<gray>The item has been added to the {crate} in prize #{prize}.", "command", "additem", "add-item-from-hand"),
    ...
    command_reload("command.reload", "{prefix}<dark_aqua>You have reloaded the Config and Data Files.", "command", "reload", "completed"),

    command_transfer_not_enough_keys("command.transfer.not.enough.keys", "{prefix}<red>You do not have enough keys to transfer.", "command", "transfer", "not-enough-keys"),
    command_transfer_sent_keys("command.transfer.sent.keys", "{prefix}<gray>You have transferred {amount} {crate} keys to {player}.", "command", "transfer", "transferred-keys"),
    command_transfer_received_keys("command.transfer.received.keys", "{prefix}<gray>You have received {amount} {crate} keys from {player}.", "command", "transfer", "transferred-keys-received"),

    command_keys_no_virtual_keys("command.keys.no.virtual.keys", "{prefix}<bold><dark_gray>(<dark_red>!<dark_gray>)</bold> <gray>You currently do not have any virtual keys.", "command", "keys", "personal", "no-virtual-keys"),

    command_keys_virtual_keys_header("command.keys.no.virtual.keys.header", StringUtils.toString(List.of(
    ...
    command_keys_target_player_no_keys("command.keys.target.player.no.keys", "{prefix}<bold><dark_gray>(<dark_red>!<dark_gray>)</bold> <gray>The player {player} does not have any keys.", "command", "keys", "other-player", "no-virtual-keys"),
    command_keys_target_player_header("command.keys.target.player.header", StringUtils.toString(List.of(
    ...
    command_keys_crate_format("command.keys.crate.format", "{crate} <bold><gray>><dark_gray>></bold> <gold>{keys} keys <gray>: Opened <gold>{crate_opened} times", "command", "keys", "crate-format"),

    command_admin_help("command.admin.help", StringUtils.toString(List.of(
    ...
    command_help("command.help", StringUtils.toString(List.of(
    ...
    must_be_player("player.must.be.player", "{prefix}<red>You must be a player to use this command.", "player", "requirements", "must-be-player"),
    must_be_console_sender("player.must.be.console.sender", "{prefix}<red>You must be using console to use this command.", "player", "requirements", "must-be-console-sender"),
    must_be_looking_at_block("player.must.be.looking.at.block", "{prefix}<red>You must be looking at a block.", "player", "requirements", "must-be-looking-at-block"),

    must_not_be_same_player("player.must.not.be.same.player", "{prefix}<red>You cannot use this command on yourself.", "player", "target-same-player"),

    must_not_be_next_to_player("player.must.not.be.next.to.player", "{prefix}<red>You are too close to a player that is opening a crate.", "player", "too-close-to-another-player"),

    inventory_not_empty("player.inventory.not.empty", "{prefix}<red>Inventory is not empty, Please make room before opening <gold>{crate}.", "player", "inventory-not-empty"),
    lacking_permission("player.lacking.permission", "{prefix}<red>You do not have permission to use that command/menu!", "player", "no-permission"),

    obtaining_keys("player.obtaining.keys", "{prefix}<gray>You have been given <gold>{amount} {key} <gray>key(s).", "player", "obtaining-keys"),

    not_online("player.not.online", "{prefix}<red>{player} <gray>is not online.", "player", "target-not-online"),
    ...
    not_physical_crate("crates.not.physical.crate", "{prefix}<gold>{crate} <red>cannot be used as a Virtual Crate.", "crates", "cannot-be-a-virtual-crate"),
    not_enough_room("crates.not.enough.room", "{prefix}<red>There is not enough space to open that here", "crates", "needs-more-room"),

    out_of_time("crates.out.of.time", "{prefix}<red>You took <gold>5 minutes <red>to open <gold>{crate} <red>so it closed.", "crates", "out-of-time"),

    world_disabled("crates.world.disabled", "{prefix}<red>I am sorry, but crates are disabled in {world}.", "crates", "world-disabled"),

    preview_force_exit("crates.preview.force.exit", "{prefix}<red>A reload has forced you out of the preview.", "crates", "forced-out-of-preview"),
    preview_disabled("crates.preview.disabled", "{prefix}<red>You cannot set the Menu to a block because the crate menu is disabled", "crates", "crate-preview-disabled"),

    crate_already_opened("crates.crate.already.open", "{prefix}<red>You are already opening <gold>{crate}.", "crates", "crate-already-open"),
    crate_already_used("crates.crate.already.used", "{prefix}<red>{crate} is already in use. Please wait until it finishes.", "crates", "crate-in-use"),

    crate_cannot_set_type("crates.cannot.set.menu.type", "{prefix}<red>You cannot set the Menu to a block because the crate menu is disabled", "crates", "cannot-set-menu-type"),

    crate_no_permission("crates.no.permission", "{prefix}<red>You do not have permission to use {crate}.", "crates", "crate-no-permission"),
    ...
    physical_crate_created("crates.physical.crate.created", StringUtils.toString(List.of(
    ...
    crate_list_per_crate("crates.list.per-crate", "<dark_gray>[<blue>{id}<dark_gray>]: <red>{crate_name}<dark_gray>, <red>{world}<dark_gray>, <red>{x}<dark_gray>, <red>{y}<dark_gray>, <red>{z}", "crates", "list", "per-crate"),
