...
public class NMS_v1_18_R1 implements NMSSupport {
    ...
    public void openChest(Block block, boolean open) {
        ...
        if (type == Material.CHEST || type == Material.TRAPPED_CHEST || type == Material.ENDER_CHEST) {
            ...
            if (block.getType() == Material.ENDER_CHEST) {
                TileEntityEnderChest tileChest = (TileEntityEnderChest) world.c_(position);
    ...
    @Override
    public List<Location> getLocations(File file, Location loc) {
        try {
            Location[] locations = StructureService.normalizeEdges(loc, StructureService.getOtherEdge(file, loc));
            int[] dimensions = StructureService.getDimensions(locations);
            return StructureService.getSingleStructureLocations(file, loc.subtract(Math.floor(dimensions[0] / 2), 1, Math.floor(dimensions[2] / 2)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public List<Material> getQuadCrateBlacklistBlocks() {
        ...
        blockList.add(Material.CRIMSON_SIGN);
        blockList.add(Material.WARPED_SIGN);
        ...
        blockList.add(Material.CRIMSON_WALL_SIGN);
        blockList.add(Material.WARPED_WALL_SIGN);

        blockList.add(Material.STONE_BUTTON);
        blockList.add(Material.POLISHED_BLACKSTONE_BUTTON);
        ...
        blockList.add(Material.CRIMSON_BUTTON);
        blockList.add(Material.WARPED_BUTTON);
        return blockList;
    }
